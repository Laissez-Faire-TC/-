<?php
// 支払い金額を決定（custom_amount が NULL なら default_amount を使用）
$amount      = $item['custom_amount'] !== null ? (int)$item['custom_amount'] : (int)$collection['default_amount'];
$isPastDeadline = $collection['deadline'] < date('Y-m-d');
...
<div class="card mb-4">
    ...
    <div class="card-body">
        <dl class="row mb-0">
            ...
            <dd class="col-7 col-sm-8 fw-bold fs-5"><?= number_format($amount) ?>円</dd>
...
document.getElementById('collectionForm')?.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        const res = await fetch('/api/member/collection-items/<?= (int)$item['id'] ?>/submit', {
            method: 'POST',
